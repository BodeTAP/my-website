...
const Logo = ({ collapsed = false }: { collapsed?: boolean }) => (
  <Link
    href="/admin"
    className={`flex items-center gap-3 group ${collapsed ? "justify-center px-0" : "px-2"}`}
...
function SidebarContent({
  ...
  return (
    <aside className="flex flex-col h-full bg-[#030914]/80 backdrop-blur-xl relative overflow-hidden">
      ...
      {/* Logo */}
      <div className={`border-b border-white/5 relative z-10 ${collapsed ? "p-4" : "p-6"}`}>
        <div className={`flex items-center ${collapsed ? "flex-col gap-3" : "justify-between gap-3"}`}>
          <Logo collapsed={collapsed} />
          <button
            type="button"
            onClick={onToggleCollapse}
            className={`${showCollapseToggle ? "hidden lg:flex" : "hidden"} w-9 h-9 items-center justify-center rounded-xl text-blue-200/50 hover:text-white bg-white/5 hover:bg-white/10 border border-white/5 transition-all`}
      ...
      {/* Nav */}
      <nav className={`flex-1 py-6 overflow-y-auto custom-scrollbar relative z-10 ${collapsed ? "px-3 space-y-5" : "px-5 space-y-8"}`}>
        {hasNoAccess ? (
          <div
          ...
          menuGroups.map((group, i) => (
            <div key={i} className={collapsed ? "space-y-1.5" : "space-y-2"}>
              <h4 className={collapsed ? "sr-only" : "px-3 text-[10px] font-black uppercase tracking-widest text-blue-200/40 mb-3"}>{group.title}</h4>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {active && !collapsed && <ChevronRight className="w-3.5 h-3.5 text-blue-500 opacity-60" />}
                    </Link>
                  );
                })}
              </div>
      ...
      {/* User / Logout */}
      <div className={`border-t border-white/5 relative z-10 bg-black/20 ${collapsed ? "p-3" : "p-5"}`}>
        <div className={`flex items-center justify-between py-1 mb-4 ${collapsed ? "px-0" : "px-1"}`}>
        ...
        >
          ...
          <span className={collapsed ? "sr-only" : ""}>Keluar Sesi</span>
        </button>
      </div>
    </aside>
  );
...
export default function AdminShell({ children, allowedModules, isSuperAdmin }: AdminShellProps) {
  ...
  return (
    <div className="min-h-screen flex bg-[#030914]">
      {/* ── Desktop sidebar ─────────────────────────────── */}
      <div
        className={`hidden lg:flex shrink-0 border-r border-white/5 flex-col z-20 shadow-[10px_0_30px_rgba(0,0,0,0.5)] transition-[width] duration-300 ease-[0.22,1,0.36,1] ${
