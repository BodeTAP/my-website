...
export async function POST(req: NextRequest) {
  ...
  if (balance < 5) {
    return NextResponse.json({ error: "Kredit tidak cukup", balance }, { status: 402 });
  }
  ...
  try {
    const { query, pages = 1, city = "" } = await req.json();
    ...
    const maxPages = Math.min(Math.max(1, Number(pages) || 1), 3);
    ...
    const deductResult = await deductCredits(
      ...
      "lead_finder",
      `Search: ${fullQuery}`,
      { query, city, results: places.length },
    );
